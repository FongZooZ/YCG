...
public class ParallaxBackground {
    ...
    public void draw(OrthographicCamera worldCamera, Batch batch) {
        ...
        for (int i = 0; i < layers.size; i++) {
            ...
            float currentX = (layer.getTileModeX().equals(TileMode.single) ? 0 : ((int) ((worldCamera.position.x - worldCamera.viewportWidth * .5f * worldCamera.zoom) / layer.getWidth())) * layer.getWidth()) - (Math.abs((1 - layer.getParallaxRatio().x) % 1) * worldCamera.viewportWidth * .5f) - layer.getWidth();
            do {
                float currentY = (layer.getTileModeY().equals(TileMode.single) ? 0 : ((int) ((worldCamera.position.y - worldCamera.viewportHeight * .5f * worldCamera.zoom) / layer.getHeight())) * layer.getHeight()) - (((1 - layer.getParallaxRatio().y) % 1) * worldCamera.viewportHeight * .5f) - layer.getHeight();
                do {
                ...
                while (currentY < worldCamera.position.y + worldCamera.viewportHeight * worldCamera.zoom * .5f + layer.getHeight());
                currentX += layer.getWidth();
                if (layer.getTileModeX().equals(TileMode.single))
                    break;
            }
            while (currentX < worldCamera.position.x + worldCamera.viewportWidth * worldCamera.zoom * .5f + layer.getWidth());
